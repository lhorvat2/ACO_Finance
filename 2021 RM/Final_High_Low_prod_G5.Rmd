...
ACO_expends = read_csv("Z:\\pgm\\ibugarinovic\\programs\\MSSP\\G5 batch\\Repayment Mechanism\\B5A19G5R.ACO19.CSV") %>% rename(ACO = ACOID)
...
qaAcoTin = as_tibble(haven::read_sas("Y:\\data\\jsciandra\\Prescreening\\2021\\G5\\batch_g5_tin_20201125.sas7bdat ")) %>%
...
temp = outputFinal_CMS %>% filter(`ACO ID` %in% c("A4756", "A4895"))
temp_parlist = qaAcoTin %>% filter(ACO == "A4756")
which(str_sub(qaAcoTin$TIN,-4,-1) == "6921")

temp_TIN_REV = qaTinRevenue %>% filter(TIN == "870716921")
...
temp_TIN_EXP = qaTinExpends %>% filter(TIN == "870716921")
...
```{r Compare with G4 output}

library(haven)


Kevin_SAS = read_sas("Z:\\pgm output\\ACOV2-5176 (Final High Low Revenue Calculations)\\Kevin\\analytic_ds_11f_out.sas7bdat")
...
#merge together
check_data<-inner_join(outputFinal_CMS,Kevin_SAS,by=c("ACO ID" = "ACO ID","ACO Participant TIN" = 'ACO Participant TIN',"Benchmark- or Revenue-Based"))
names(check_data)

#This should have zero rows. Datasets should have same ACO/TIN combinations
total_match<-anti_join(outputFinal_CMS,Kevin_SAS,by=c("ACO ID" = "ACO ID","ACO Participant TIN" = 'ACO Participant TIN',"Benchmark- or Revenue-Based"))
...
#merge together
check_data<-inner_join(outputFinal_CMS,POR,by=c("ACO ID" = "ACO ID","ACO Participant TIN" = 'ACO Participant TIN',"Benchmark- or Revenue-Based"))
names(check_data)

#This should have zero rows. Datasets should have same ACO/TIN combinations
total_match<-anti_join(outputFinal_CMS,POR,by=c("ACO ID" = "ACO ID","ACO Participant TIN" = 'ACO Participant TIN',"Benchmark- or Revenue-Based"))
nrow(total_match)
...
Summary_tab = read_excel("Z:\\participant management\\prescreening\\2021 Prescreening\\Batch G5\\Participation Options Report\\Participation_Options_PY2021_Full_Assignment_20201201.xlsx", sheet=2)
...
```{r Compare G4 to G5 output and write out individual files for ACOs that require an RM increase}
unique(POR$`Repayment Mechanism Amount Action Required`)

ACOs_need_increase_G5 = POR %>% filter(`Repayment Mechanism Amount Action Required` == "Contact your ACO coordinator, ACO met threshold for potential RM increase") %>%  filter(`ACO Participant TIN` == "Total/Average" & `Benchmark- or Revenue-Based` == "Benchmark-based") %>% select(`ACO ID`, `Required Repayment Mechanism Amount`, `High or Low Revenue ACO Determination`)

ACOs_need_increase_G4 = read_excel("Z:\\participant management\\prescreening\\2021 Prescreening\\Batch G4\\Participation Options Report\\Participation_Options_PY2021_Full_Assignment_20201123.xlsx", sheet = 1) %>% filter(`Repayment Mechanism Amount Action Required` == "Contact your ACO coordinator, ACO met threshold for potential RM increase") %>%  filter(`ACO Participant TIN` == "Total/Average" & `Benchmark- or Revenue-Based` == "Benchmark-based") %>% select(`ACO ID`, `Required Repayment Mechanism Amount`, `High or Low Revenue ACO Determination`)
Compare_G5_G4_need_increase = ACOs_need_increase_G5 %>% inner_join(ACOs_need_increase_G4, by = "ACO ID") %>% mutate(match_hi_low = (`High or Low Revenue ACO Determination.x` == `High or Low Revenue ACO Determination.y`),
                                                                                                         diff_req_rm = (`Required Repayment Mechanism Amount.y` - `Required Repayment Mechanism Amount.x`) / `Required Repayment Mechanism Amount.y`
)


G5 = POR %>%   filter(`ACO Participant TIN` == "Total/Average" & `Benchmark- or Revenue-Based` == "Benchmark-based") %>% select(`ACO ID`, `Required Repayment Mechanism Amount`, `High or Low Revenue ACO Determination`)
G4 = read_excel("Z:\\participant management\\prescreening\\2021 Prescreening\\Batch G4\\Participation Options Report\\Participation_Options_PY2021_Full_Assignment_20201123.xlsx", sheet = 1)  %>%  filter(`ACO Participant TIN` == "Total/Average" & `Benchmark- or Revenue-Based` == "Benchmark-based") %>% select(`ACO ID`, `Required Repayment Mechanism Amount`, `High or Low Revenue ACO Determination`)

Compare_G5_G4 = G5 %>% inner_join(G4, by = "ACO ID") %>% mutate(match_hi_low = (`High or Low Revenue ACO Determination.x` == `High or Low Revenue ACO Determination.y`),
                                                                                                         diff_req_rm = (`Required Repayment Mechanism Amount.y` - `Required Repayment Mechanism Amount.x`) / `Required Repayment Mechanism Amount.y`
)
...
#POR_1 =  read_excel("Z:\\participant management\\prescreening\\2021 Prescreening\\Batch G5\\Participation Options Report\\Participation_Options_PY2021_Full_Assignment_20201201.xlsx", sheet=1)%>% 
#  mutate(`ACO Participant TIN` = str_pad(`ACO Participant TIN`, 9, side = c("left"), pad = "0"))

#POR_2 = read_excel("Z:\\participant management\\prescreening\\2021 Prescreening\\Batch G5\\Participation Options Report\\Participation_Options_PY2021_Full_Assignment_20201201.xlsx", sheet=2)
#library(openxlsx)
#for (ACO in ACOs_need_increase_G5$`ACO ID`){
# POR_1_filter = POR_1 %>% filter(`ACO ID` == ACO) 
# POR_2_filter = POR_2 %>% filter(`ACO ID` == ACO)
# wb <- createWorkbook()
# addWorksheet(wb, "Estimate of High or Low Revenue")
# writeData(wb, sheet = "Estimate of High or Low Revenue", POR_1_filter)
# setColWidths(wb, sheet = 1, cols = 1:ncol(POR_1_filter), widths = "auto")
 
# addWorksheet(wb, "ACO Summary")
# writeData(wb, sheet = "ACO Summary", POR_2_filter)
# setColWidths(wb, sheet = 2, cols = 1:ncol(POR_2_filter), widths = "auto")
 
 #datasets = list("Estimate of High or Low Revenue" = POR_1_filter, "ACO Summary" = POR_2_filter)
# path = paste0("Z:\\participant management\\prescreening\\2021 Prescreening\\Batch G5\\Participation Options Report\\Participation_Options_PY2021_Full_Assignment_20201201","_",ACO,".xlsx")
 #openxlsx::write.xlsx(datasets, file = path) 
