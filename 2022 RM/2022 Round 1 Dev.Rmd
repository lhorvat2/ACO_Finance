...
repay <- read_excel("Z:\\finance\\participant revenue and expenditures\\2022 Cycle\\Test Cases\\Repay_Current.xlsx") %>%
...
qaAcoTin = as_tibble(readxl::read_excel("Z:\\finance\\participant revenue and expenditures\\2022 Cycle\\Test Cases\\2022TestCase.xlsx")) %>%
...
qaAcoTinCcn <- as_tibble(haven::read_sas("Y:\\data\\mmills\\4_prescreening\\2021\\BatchG2\\ccn_list\\g2ccn20200828.sas7bdat")) %>%
  filter(ACO %in% qaAcoTin$ACO) %>% 
  filter(use_record == 1) %>%
  select(ACO , TIN, CCN, use_record) %>%
...
qaAcoTinCcn %>% mutate(use_record = 1) %>%  select(ACO, TIN,CCN, use_record) %>% openxlsx::write.xlsx("Z:\\finance\\participant revenue and expenditures\\2022 Cycle\\Test Cases\\ccn_list.xlsx")



```

```{r Read in new inputs for 2022}

# Prior Legal Entity
Prior_Legal_Entity = read_csv("Z:\\finance\\participant revenue and expenditures\\2022 Cycle\\Test Cases\\prior_legal_entity_TC.csv")
...
qaAllBnchmk <- full_join(qaAcoTinAll, 
  ...
  # e ix 
  left_join(Prior_Legal_Entity, by = c("ACO" = "ACO_ID")) %>% 
  # f
  left_join(repay, by = c("PRIOR_LEGAL_ACO" = "ACO")) %>% 
  
  arrange(ACO, desc(TIN)) %>% 
  rename(REPAY_CURRENT = REPAY_CURRENT.x,
         PRIOR_ACO_RM = REPAY_CURRENT.y)
...
initDeterFn = Vectorize(function(ASSIGNED, TRACK, FCST_RM_AMT_EXP, FCST_RM_AMT_REV, SCENARIO){
  INITIAL_DETER_RM_AMT = c()
  INITIAL_SCENARIO=c()
  if(ASSIGNED > 0 ) {
  INITIAL_DETER_RM_AMT = min(FCST_RM_AMT_EXP, FCST_RM_AMT_REV)
  INITIAL_SCENARIO="Positive Assignment"
  }
  else{
    INITIAL_DETER_RM_AMT = FCST_RM_AMT_REV
    INITIAL_SCENARIO="0 Assignment"
  }
  if (SCENARIO==TRUE){
...
ACO_category_fn = Vectorize(function(Latest_Agreement_Start_Date, RISK, INTO_RISK){
  ...
  if(Latest_Agreement_Start_Date %in% c("2019-07-01", "2020-01-01") & RISK == "Yes" & INTO_RISK==1){
    return("into_two_sided")
  }
  else if(Latest_Agreement_Start_Date %in% c("2019-07-01", "2020-01-01") & RISK == "Yes" & INTO_RISK==0){
    return("maintain_two_sided")
  }  
  else if(Latest_Agreement_Start_Date %in% c("2022-01-01")  & RISK == "Yes"){
    return("applicant_two_sided")
  }
  else{
...
REQ_ACTION_THRESHOLD_fn = Vectorize(function(ACO_CATEGORY, RENEWING_ACO, REPAY_CURRENT, INITIAL_DETER_RM_AMT, TRACK, variable, PRIOR_LEGAL_ACO, PRIOR_ACO_RM){
##  Basic/Enhanced Two Sided ACOs maintaining risk
#    •	Scenario 1: ACO_CATEGORY= “maintain_two_sided”  [(INITIAL_DETER_RM_AMT >= 1.5 x REPAY_CURRENT) OR (INITIAL_DETER_RM_AMT >= (1,000,000 + REPAY_CURRENT))]
...
#   •	Scenario 2:  ACO_CATEGORY= “maintain_two_sided” (INITIAL_DETER_RM_AMOUNT < 1.5 x REPAY_CURRENT) & (INITIAL_DETER_RM_AMOUNT < (1,000,000 + REPAY_CURRENT))
...
#      o	THRESHOLD_EXCEEDED = "No"
    ...
    if(ACO_CATEGORY=="maintain_two_sided" & !is.na(REPAY_CURRENT) & (INITIAL_DETER_RM_AMT >= 1.5 * REPAY_CURRENT | INITIAL_DETER_RM_AMT >= (1E6 + REPAY_CURRENT))){
    ...
    REQ_SCENARIO = 'maintain_two_sided_1'
    }
    else if(ACO_CATEGORY=="maintain_two_sided" & !is.na(REPAY_CURRENT) & (INITIAL_DETER_RM_AMT < 1.5 * REPAY_CURRENT & INITIAL_DETER_RM_AMT < (1E6 + REPAY_CURRENT))){
    REQ_RM_AMT = REPAY_CURRENT
    Message = "None. No change needed to existing RM amount"
    Threshold = "No"
    REQ_SCENARIO = 'maintain_two_sided_2'
    }
    
    else if(ACO_CATEGORY=="maintain_two_sided" & is.na(REPAY_CURRENT) ){
    REQ_RM_AMT = INITIAL_DETER_RM_AMT
    Message = "Establish RM at required RM amount"
    Threshold = "Not Applicable"
    REQ_SCENARIO = 'maintain_two_sided_3'
    }
 ...
 ##  Basic/Enhanced Two Sided ACOs moving into risk
  #
  #•	Scenario 1: ACO_CATEGORY= “into_two_sided” 
    #o	REQ_RM_AMT = INITIAL_DETER_RM_AMT
    #o	RM_ACTION = "Establish RM at required amount"
    #o	THRESHOLD_EXCEEDED = "Not Applicable"
  
    else if(ACO_CATEGORY=="into_two_sided"){
    REQ_RM_AMT = INITIAL_DETER_RM_AMT
    Message = "Establish RM at required RM amount"
    Threshold = "Not Applicable"
    REQ_SCENARIO = 'into_two_sided_1'
    }
  ...
  ## Applicant Two-Sided ACOs
  #•	Scenario 1: ACO_CATEGORY= “applicant_two_sided ” & RENEWING_ACO="Yes" & REPAY_CURRENT is not missing & INITIAL_DETER_RM_AMT > =REPAY_CURRENT
    #o	REQ_RM_AMT = INITIAL_DETER_RM_AMT
    #o	RM_ACTION = “Increase existing RM to required RM amount."
    #o	THRESHOLD_EXCEEDED = "Not Applicable"

  #•	Scenario 2: ACO_CATEGORY= “applicant_two_sided ” & RENEWING_ACO="Yes" & REPAY_CURRENT is not missing & INITIAL_DETER_RM_AMT <REPAY_CURRENT
    #o	REQ_RM_AMT = INITIAL_DETER_RM_AMT
    #o	RM_ACTION = “Required RM amount is less than existing RM amount"
    #o	THRESHOLD_EXCEEDED = "Not Applicable"
      
  #•	Scenario 3: ACO_CATEGORY= “applicant_two_sided ” & RENEWING_ACO="Yes" & REPAY_CURRENT is missing 
    #o	REQ_RM_AMT = INITIAL_DETER_RM_AMT
    #o	RM_ACTION = “Establish RM at required amount"
    #o	THRESHOLD_EXCEEDED = "Not Applicable"
      
  #•	Scenario 4: ACO_CATEGORY= “applicant_two_sided ” & RENEWING_ACO="No” &  PRIOR_LEGAL_ACO is not missing & PRIOR_ACO_RM is not missing
    #o	REQ_RM_AMT = INITIAL_DETER_RM_AMT
    #o	RM_ACTION = “Contact your ACO coordinator. ACO may be eligible to utilize RM from prior legal entity"
    #o	THRESHOLD_EXCEEDED = "Not Applicable"
      
  #•	Scenario 5: ACO_CATEGORY= “applicant_two_sided ” & RENEWING_ACO="No” &  PRIOR_LEGAL_ACO is not missing & PRIOR_ACO_RM is missing
    #o	REQ_RM_AMT = INITIAL_DETER_RM_AMT
    #o	RM_ACTION = “Establish RM at required amount"
    #o	THRESHOLD_EXCEEDED = "Not Applicable"
      
  #•	Scenario 6: ACO_CATEGORY= “applicant_two_sided ” & RENEWING_ACO="No” &  PRIOR_LEGAL_ACO is missing 
    #o	REQ_RM_AMT = INITIAL_DETER_RM_AMT
    #o	RM_ACTION = “Establish RM at required amount"
    #o	THRESHOLD_EXCEEDED = "Not Applicable"


    else if(ACO_CATEGORY=="applicant_two_sided" & RENEWING_ACO == "Yes" & !is.na(REPAY_CURRENT) & INITIAL_DETER_RM_AMT >= REPAY_CURRENT ){
    REQ_RM_AMT = INITIAL_DETER_RM_AMT
    Message = "Increase existing RM to required RM amount"
    Threshold = "Not Applicable"
    REQ_SCENARIO = 'applicant_two_sided_1'
    }

    else if(ACO_CATEGORY=="applicant_two_sided" & RENEWING_ACO == "Yes" & !is.na(REPAY_CURRENT) & INITIAL_DETER_RM_AMT < REPAY_CURRENT ){
    REQ_RM_AMT = INITIAL_DETER_RM_AMT
    Message = "Required RM amount is less than existing RM amount"
    Threshold = "Not Applicable"
    REQ_SCENARIO = 'applicant_two_sided_2'
    }
  
    else if(ACO_CATEGORY=="applicant_two_sided" & RENEWING_ACO == "Yes" & is.na(REPAY_CURRENT)){
    REQ_RM_AMT = INITIAL_DETER_RM_AMT
    Message = "Establish RM at required amount"
    Threshold = "Not Applicable"
    REQ_SCENARIO = 'applicant_two_sided_3'
      }
  
    else if(ACO_CATEGORY=="applicant_two_sided" & RENEWING_ACO == "No" & !is.na(PRIOR_LEGAL_ACO) & !is.na(PRIOR_ACO_RM)){
    REQ_RM_AMT = INITIAL_DETER_RM_AMT
    Message = "Contact your ACO coordinator. ACO may be eligible to utilize RM from prior legal entity"
    Threshold = "Not Applicable"
    REQ_SCENARIO = 'applicant_two_sided_4'
        }
    ...
    Message = "Establish RM at required amount"
    Threshold = "Not Applicable"
    REQ_SCENARIO = 'applicant_two_sided_5'
          }
  
    else if(ACO_CATEGORY=="applicant_two_sided" & RENEWING_ACO == "No" & is.na(PRIOR_LEGAL_ACO)){
    REQ_RM_AMT = INITIAL_DETER_RM_AMT
    Message = "Establish RM at required amount"
    Threshold = "Not Applicable"
    REQ_SCENARIO = 'applicant_two_sided_6'
    }
...
## non risk ACOs    
  #•	Scenario 1: ACO_CATEGORY = “non_two_sided” 
    #o	  REQ_RM_AMT = INITIAL_DETER_RM_AMT
    #o	  RM_ACTION = "For Information Only. No RM Adjustment Required"
    #o	  THRESHOLD_EXCEEDED = "Not Applicable" 
...
output <- qaAll %>%
  ...
  mutate(ASSG_GROWTH = 1,
         ...
         INITIAL_DETER_RM_AMT = initDeterFn(ASSIGNED, TRACK, FCST_RM_AMT_EXP, FCST_RM_AMT_REV, SCENARIO=FALSE),
         
         INITIAL_SCENARIO = initDeterFn(ASSIGNED, TRACK, FCST_RM_AMT_EXP, FCST_RM_AMT_REV, SCENARIO=TRUE),
         
         RENEWING_ACO =  if_else(ACO_START_DATE != "2022-01-01" & Latest_Agreement_Start_Date=="2022-01-01","Yes","No"),

         ACO_CATEGORY = ACO_category_fn(Latest_Agreement_Start_Date, RISK, INTO_RISK),
         
         REQ_RM_AMT = REQ_ACTION_THRESHOLD_fn(ACO_CATEGORY, RENEWING_ACO, REPAY_CURRENT, INITIAL_DETER_RM_AMT, TRACK,variable='REQ_RM_AMT', PRIOR_LEGAL_ACO, PRIOR_ACO_RM),
         
         RM_ACTION = REQ_ACTION_THRESHOLD_fn(ACO_CATEGORY, RENEWING_ACO, REPAY_CURRENT, INITIAL_DETER_RM_AMT,  TRACK, variable='Message', PRIOR_LEGAL_ACO, PRIOR_ACO_RM),
         
         THRESHOLD_EXCEEDED  = REQ_ACTION_THRESHOLD_fn(ACO_CATEGORY, RENEWING_ACO, REPAY_CURRENT, INITIAL_DETER_RM_AMT, TRACK,variable='Threshold', PRIOR_LEGAL_ACO, PRIOR_ACO_RM),
         
         REQ_SCENARIO = REQ_ACTION_THRESHOLD_fn(ACO_CATEGORY, RENEWING_ACO, REPAY_CURRENT, INITIAL_DETER_RM_AMT, TRACK, variable='Req_Scenario', PRIOR_LEGAL_ACO, PRIOR_ACO_RM),
...
#test_3 = output %>% filter(ACO %in% check_initial$`ACO ID`)
...
# Blanking select columns for non-risk ACOs
      #10a.    If ACO_CATEGORY in (A,B)  set the following variables equal to missing:
         ##i.	REPAY_CURRENT
         ##ii.	INITIAL_DETER_RM_AMT
         ##iii.	REQ_RM_AMT
blankRM <- Vectorize(function(TRACK, input){
  if(TRACK %in% c("A","B")){return(NA)}
  else{return(input)}
})
...
outputBlanked <- output %>%
  mutate(
         ...
         # Blanking select columns for non-risk ACOs
         REPAY_CURRENT = blankRM(TRACK, REPAY_CURRENT),
         INITIAL_DETER_RM_AMT = blankRM(TRACK, INITIAL_DETER_RM_AMT),
         REQ_RM_AMT = blankRM(TRACK, REQ_RM_AMT),
...
#9e.	For all rows in dataset, set PROGRAM_YEAR = 2022
...
# name variables
  select("ACO ID" = ACO, 
         ...
         Track = TRACK,
         "2020 Assigned Beneficiaries" = ASSIGNED,
         "Assignment Growth" = ASSG_GROWTH,
         "Projected 2022 Assigned Beneficiaries" = PROJ_ASSIGNED,
         "2020 Dollars per Beneficiary" = DOLLAR_PER_BENE,
         "Dollar Trend" = DOLLAR_TREND,
         "Projected 2022 Dollars per Beneficiary" = PROJ_DOLLAR_PER_BENE,
         "Projected 2022 Total Dollars" = PROJ_TOT_DOLLAR,
         "Required Percentage" = REQ_PCT, 
         "Forecast Repayment Mechanism Amount" = FCST_RM_AMT,
         "Performance-Based Risk Track" = RISK,
         "Existing Repayment Mechanism Amount" = REPAY_CURRENT,
         "Initial Determined Repayment Mechanism Amount" = INITIAL_DETER_RM_AMT,
         "Met Threshold to Update Repayment Mechanism Amount" = THRESHOLD_EXCEEDED,
         "Required Repayment Mechanism Amount" = REQ_RM_AMT,
         "Repayment Mechanism Amount Action Required" = RM_ACTION,
         "Revenue as Percent of Expenditures" = HI_LOW_RATIO,
         "High or Low Revenue ACO Determination" = ACO_HI_LOW_REV
         )



#replace improper Hi Low revenue value
outputFinal$`Revenue as Percent of Expenditures`[outputFinal$`Revenue as Percent of Expenditures`==Inf] = NA

#don't need steps 10d or 9g because these are the data export steps


outputFinal %>% openxlsx::write.xlsx("Z:\\finance\\participant revenue and expenditures\\2022 Cycle\\Test Cases\\Luke Output\\Step_10.xlsx")
...
#replace improper Hi Low revenue value
outputFinal_CMS$`Revenue as Percent of Expenditures`[outputFinal_CMS$`Revenue as Percent of Expenditures`==Inf] = NA

#don't need step 9g because these are the data export steps


outputFinal_CMS %>% openxlsx::write.xlsx("Z:\\finance\\participant revenue and expenditures\\2022 Cycle\\Test Cases\\Luke Output\\Step_9.xlsx")
...
Alex_data = read_excel("Z:\\finance\\participant revenue and expenditures\\2022 Cycle\\Test Cases\\Alex Output\\Step10Excel.xls", sheet=1) 
names(Alex_data)
...
check_data<-check_data %>% 
  mutate(check_assigned=`2020 Assigned Beneficiaries`==`ASSIGNED` ,
         
         check_assg_growth=round(`Assignment Growth`,1)==round(`ASSG_GROWTH`,1),
         
         #had to adjust rounding here
         check_proj_2022_benes=if_else(`Projected 2022 Assigned Beneficiaries` == 0, `Projected 2022 Assigned Beneficiaries` == `PROJ_ASSG`, abs(Delt(`Projected 2022 Assigned Beneficiaries`,round(`PROJ_ASSG`,0))) < .0001),
         
         check_Doll_per_bene=abs(Delt(as.numeric(`2020 Dollars per Beneficiary`),as.numeric(`DOLLAR_PER_BENE`)))<.001,
         
         check_trend_dollar=round(as.numeric(`Dollar Trend`),1)==round(as.numeric(`DOLLAR_TREND`),1),
         
         check_Proj_Doll_per_bene=abs(Delt(as.numeric(`Projected 2022 Dollars per Beneficiary`),as.numeric(`PROJ_DOLLAR_PER_BENE`)))<.001,
         
         #had to adjust rounding here
         check_Proj_Total_Dollars= if_else(`Projected 2022 Total Dollars` == 0, `Projected 2022 Total Dollars` == `PROJ_TOT_DOLLAR`, abs(Delt(as.numeric(`Projected 2022 Total Dollars`),as.numeric(round(`PROJ_TOT_DOLLAR`,2))))<.0001),
         
         check_req_perc=round(`Required Percentage`,1)==round(`REQ_PCT`,1),
         
         #had to adjust rounding here
         check_forecast_repay=if_else(`Forecast Repayment Mechanism Amount` == 0, `Forecast Repayment Mechanism Amount` == round(`FCST_RM_AMT`,0),abs(Delt(`Forecast Repayment Mechanism Amount`,round(`FCST_RM_AMT`,0)))<.005),
         
         check_risk_track=`Performance-Based Risk Track`==`RISK`,
         
         check_existing_repay_amt=round(as.numeric(`Existing Repayment Mechanism Amount`),0)==round(as.numeric(`REPAY_CURRENT`),0),
         
         check_Initial_Repay_amt=abs(Delt(as.numeric(`Initial Determined Repayment Mechanism Amount`),as.numeric(`INITIAL_DETER_RM_AMT`)))<.0001,
         
         check_met_threshold = `Met Threshold to Update Repayment Mechanism Amount` == `THRESHOLD_EXCEEDED`,
         
         check_required_repay_amt=abs(Delt(as.numeric(`Required Repayment Mechanism Amount`),as.numeric(`REQ_RM_AMT`)))<.0001,
         
         check_action = `Repayment Mechanism Amount Action Required` == `RM_ACTION`,
         
         check_action_lower = action_x == action_y,
         
         check_rev_perc_exp=`Revenue as Percent of Expenditures`- as.numeric(`HI_LOW_RATIO`)<=.001,
         
         check_hi_low_det=`High or Low Revenue ACO Determination`==`ACO_HI_LOW_REV`)
...
check_forecast = check_data %>% filter(check_forecast_repay == FALSE)
check_action = check_data %>% filter(check_action_lower == FALSE)

test = qaAcoTin %>% filter(ACO == "A3644")
test_2 = qaAcoTinCcn %>% filter(ACO == "A2272")
